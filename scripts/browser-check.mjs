...
try {
 ...
 assert.equal(await page.locator('.material:visible').count(),22);
 await page.locator('#study-month').selectOption('2025-11');
 assert.equal(await page.locator('.material:visible').count(),2);
 await page.locator('[data-filter="docs"]').click();
 assert.equal(await page.locator('.material:visible').count(),1);
 await page.locator('[data-course-link="docs"]').click();
 assert.equal(await page.locator('.material:visible').count(),8);
 assert.equal(await page.locator('#study-month').inputValue(),'');
 await page.locator('[data-course-link="ui"]').click();
 assert.equal(await page.locator('.material:visible').count(),8);
 ...
 assert.equal(await page.locator('#complete-lesson').getAttribute('aria-pressed'),'true');
 await page.goto('http://127.0.0.1:4173/');
 assert.equal(await page.locator('#progress-percent').innerText(),'5%');
 ...
 await page.goto('http://127.0.0.1:4173/lessons/ui-8.html');
 assert.equal(await page.locator('time.study-date').getAttribute('datetime'),'2026-07-08');
 assert.equal(await page.locator('.nav-date').count(),8);
 assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth),'New practice mobile overflow');
 await page.locator('.quiz input[value="1"]').check();
 await page.locator('.quiz button').click();
 assert.match(await page.locator('.quiz-result').innerText(),/^Верно/);
 await page.locator('#complete-lesson').click();
 await page.goto('http://127.0.0.1:4173/');
 assert.equal(await page.locator('#progress-percent').innerText(),'9%');
 await page.evaluate(()=>localStorage.setItem('dadakhaev-study-progress-v1',JSON.stringify(['docs-1','cad-3'])));
 await page.reload();
 assert.equal(await page.locator('#progress-text').innerText(),'Изучено 2 из 22 тем');
 assert.ok(await page.locator('[data-completed="docs-1"]').isVisible());
 assert.ok(await page.locator('[data-completed="cad-3"]').isVisible());
